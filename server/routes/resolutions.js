  ...
  try {
    ...
    const stmt = db.prepare(`
      INSERT INTO resolutions (
        user_id, title, description, why, category, goal_value, current_value, unit, deadline, status, completed_at
      )
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
    ...
    // Initial creation bonus XP (+10 XP), plus completion XP (+100 XP) if already completed
    let xpAward = 10;
    if (isCompleted) {
      xpAward += 100;
    }
...
router.put('/:id', (req, res) => {
  try {
    ...
    const updateStmt = db.prepare(`
      UPDATE resolutions
      SET title = ?, description = ?, why = ?, category = ?, goal_value = ?, current_value = ?,
          unit = ?, deadline = ?, status = ?, completed_at = ?, updated_at = CURRENT_TIMESTAMP
